...
public class PokemonDAO {
    ...
    public PokemonTO save(PokemonTO pokemon) {
        String sql = "insert into ddd_pokemon (nome, altura, peso, categoria, data_da_captura) values(?, ?, ?, ?, ?)";
        try(PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql))
        {
    ...
    public PokemonTO update(PokemonTO pokemon) {
        String sql = "update ddd_pokemon set nome=?, altura=?, peso=?, categoria=?, data_da_captura=? where codigo=?";
        try (PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)){
            ps.setString(1, pokemon.getNome());
            ps.setDouble(2, pokemon.getAltura());
            ps.setDouble(3, pokemon.getPeso());
